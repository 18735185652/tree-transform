...
export default class TreeTransfer extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      selectValues: this.props.values
        ? this.props.values
        : this.props.defaultValues, // 最后选择到右侧的values(values的优先级高于defaultValues)
      leftTree: {
        // 左侧剩余的数据
        dataSource: [], // 展示的数据
  ...
  operationOnCheck = (keys, data, direction, rightToLeft, callback) => {
    // console.log('666newdata: ', data);
    // console.log('666keys: ', keys);
    ...
    console.log('666newData: ', newData);
    ...
    if (direction === 'left') {
      this.setState(
        {
          leftTree: {
            ...
            checkedKeys: rightToLeft
              ? this.state.leftTree.checkedKeys
              : lastLevelKey,
            ...
            keys: rightToLeft
              ...
              : _.uniq([...this.state.selectValues, ...lastLevelKey]),
          },
        },
        () => {
          const y = [...lastLevelKey, ...this.state.selectValues];
          const newKeys = _.uniq(y);

          this.operationOnCheck(
            newKeys,
            dataSource,
            direction,
            rightToLeft,
      ...
      this.setState(
        ...
        () =>
          this.operationOnCheck(
            lastLevelKey,
            this.state.rightTree.dataSource,
            direction,
            rightToLeft,
          ),
      );
  ...
  checkAll = (direction, type) => {
    ...
    const selectAllKeys = getLastLevelData(
      this.state[operationState].dataSource,
    ).map((item) => item.key);
    // 全选右侧时所有的key
    const allRightTreeKeys = getLastLevelData(
      this.state.rightTree.dataSource,
    ).map((item) => item.key);
    // 全选左侧时所有的key
    const allKeys = getLastLevelData(this.state.dataSource).map(
      (item) => item.key,
    );
    // 根据选择的方向生成对应的key
    ...
    this.setState({
      [operationState]: {
        ...
        selectDataSource: directionDisabled
          ? disabledCategoryData(this.state.dataSource)
  ...
  render() {
    const { leftTree, rightTree } = this.state;
    const { title, showSearch, searchPlaceholder, notFoundContent } =
      this.props;
          ...
          {_.isEmpty(leftTree.dataSource) ? (
            ...
            <div className="dyx-transfer-tree">
              <Tree
                ...
                checkable
                onCheck={(keys, info) =>
                  this.onCheck(keys, info, 'left', false)
                }
          ...
          {_.isEmpty(rightTree.dataSource) ? (
            ...
            <div className="dyx-transfer-tree">
              <Tree
                ...
                checkable
                onCheck={(keys, info) =>
                  this.onCheck(keys, info, 'right', false)
                }
                checkedStrategy="child"
